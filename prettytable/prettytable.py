...
class PrettyTable(object):
    def __init__(self, field_names=None, **kwargs):
        ...
        # Options
        self._options = "title title_align start end fields header border sortby reversesort sort_key attributes format hrules vrules".split()
        ...
        self._title_align = kwargs["title_align"] or 'c'
    ...
    def title_align(self, val):
        self._title_align = self._unicode(val)
    ...
    def _compute_widths(self, rows, options):
        ...
        if self._min_table_width or options["title"]:
            if options["title"]:
                title_width = 0
                for line in options["title"].splitlines():
                    title_width = max(title_width, _get_size(line)[0] + sum(self._get_padding_widths(options)))
            ...
            if table_width < min_width:
                ...
                scale = 1.2 * min_width / table_width
    ...
    def get_string(self, **kwargs):
        ...
        title - optional table title
        title_align - optional table title alignment
        start - index of first data row to include in output
        end - index of last data row to include in output PLUS ONE (list slice style)
    ...
    def _stringify_title(self, title, options):
        ...
        for line in title.splitlines():
            ...
            title = " " * lpad + line + " " * rpad
            bits.append(self._justify(title, len(self._hrule) - 2, self.title_align))
            bits.append(endpoint)
